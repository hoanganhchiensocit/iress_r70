...
function runSpring(
	...
	return [
		...
		cond(state.finished, [
			stopClock(clock),
			call([state.position], ([a]) => onDone && onDone(a))
		]),
		state.position
	];
}
...
export default class BottomSheetBehavior extends Component {
	...
	constructor(props) {
		...
		for (let i = 1; i < snapPoints.length; i++) {
			middlesOfSnapPoints.push(
				divide(add(snapPoints[i - 1], snapPoints[i]), 2)
			);
		}
			...
			cond(
				...
				[
					...
					set(
						...
						add(
							masterOffseted,
							sub(this.dragMasterY, prevMasterDrag)
						)
					),
		...
		this.backdropOpacity = Animated.sub(
			0.8,
			Animated.multiply(
				divide(
	...
	withEnhancedLimits(val, masterOffseted, masterClock) {
		...
		return block([
			...
			cond(
				...
				[
					...
					cond(
						not(eq(this.panState, State.END)),
					...
					cond(
						and(
							...
							not(clockRunning(masterClock)),
							greaterOrEq(
								abs(sub(masterOffseted, this.snapPoint)),
								3
							)
						),
						[
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (state && state.snapPoints) {
			...
			snapPoints = sortedPropsSnapPints.map((p, i) => {
				if (i === 0) {
					return props.top
						? new Value(props.top)
						: new Value(sortedPropsSnapPints[0].val - p.val);
				} else {
					return new Value(sortedPropsSnapPints[0].val - p.val);
				}
			});
		}

		const propsToNewIncides = {};
		sortedPropsSnapPints.forEach(
			({ ind }, i) => (propsToNewIncides[ind] = i)
		);
		...
		return {
			...
			heightOfHeaderAnimated:
				(state && state.heightOfHeaderAnimated) ||
				new Animated.Value(0),
			heightOfContent:
				(state && state.heightOfContent) || new Animated.Value(0),
				...
				>
					...
					<KeyboardAvoidingView enabled={false} behavior="height">
						<View
							style={{
								height:
									this.props.snapPoints[0] - heightOfHeader,
								overflow: 'hidden'
							}}
						>
