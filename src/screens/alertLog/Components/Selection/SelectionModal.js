...
const useShowOrHide = function ({ opacity }) {
	const startCallback = _.throttle((cb) => { // start callback function call twice??
		cb && cb();
	}, 350, { trailing: false, leading: true });
...
const Selection = () => {
	const { layoutInput, onSelected, onClose, data, selectedValue, styleValue } = useRoute().params || {};
