...
import {
    View, Text, TouchableOpacity, ScrollView, Dimensions, Animated, Image,
...
import { DragIcon, ReloadIcon, CloseIcon } from './components/Icon'
...
export default class AlertSlidingPanel extends XComponent {
    ...
    getDragableTop() {
        const heightAndroid = Controller.getRealWindowHeight()
        this.dic.top = Platform.OS === 'ios'
            ? FunctionUtil.isIphoneXorAbove()
                ? height - 48 - 38 // fix tinh toan height. height expect = height man hinh - height nav - heightStatusBar
                : height - 64
            : heightAndroid - 48 - StatusBarManager.HEIGHT
        return this.dic.top
    }

    getDragableMiddle() {
        const height = Controller.getRealWindowHeight()
        this.dic.middle = (Platform.OS === 'ios'
            ? FunctionUtil.isIphoneXorAbove()
                ? height - 48 - 38
                : height - 64
            : height - 48 - StatusBarManager.HEIGHT) / 2
    ...
    componentWillUnmount() {
        ...
        this.dic.animatedValue.removeListener(this.onAnimatedValueChange);
    }
    ...
    onAnimatedValueScrollChange({ value }) {
        ...
        // }
        if (this.dic.valueTrigger) {
            const threshold = value - this.dic.valueTrigger
            if (threshold > 22) {
                this.dic.refAlertPriceHeader && this.dic.refAlertPriceHeader.current.updatePriceHeader(false)
            } else if (threshold < 18) {
                this.dic.refAlertPriceHeader && this.dic.refAlertPriceHeader.current.updatePriceHeader(true)
            }
        }
    }
    ...
    show(data) {
        this.dic.nestedScroll && this.dic.nestedScroll.showContainer()
    }
    ...
    getSnapshot({ symbolObj = {}, isGoTop = true, cb }) {
        ...
        // this.pubDataToAlertNullPrice()
        this.pubSymbolToNullpoint(true)
        // this.pubSymbolToNullpoint()
        const { symbol, displayName, company, symbolClass } = symbolObj
    ...
    onClose() {
        ...
        this.dic.nestedScroll && this.dic.nestedScroll.hideContainer();
    }
    ...
    setValueTrigger = (value) => {
        if (value) {
            this.dic.valueTrigger = value
        }
    }
    renderSlidingV2 = () => {
        return (
            <NestedScrollViewV2
                scrollContentValue={this.dic.scrollValue}
                setValueBeginTriggerScroll={this.setValueTrigger}
                ref={ref => {
                    this.dic.nestedScroll = ref
                }}
                style={{ flex: 1, backgroundColor: CommonStyle.backgroundColor }}
            >
                <View isAbsolute style={{ overflow: 'hidden' }}>
                    <Image source={pinBackground} style={{ width: '100%', position: 'absolute', top: 20, bottom: 0 }} resizeMode={'cover'} />
                    <View style={{ backgroundColor: CommonStyle.backgroundColor, borderTopLeftRadius: 16, borderTopRightRadius: 16, borderBottomRightRadius: CommonStyle.borderBottomRightRadius, paddingBottom: 16 }}>
                        <AlertPriceHeader
                            ref={this.dic.refAlertPriceHeader}
                            onClose={this.onClose}
                            onC2R={this.props.onC2R}
