...
export class WatchListSearchNew extends PureComponent {
	...
	renderTitle(item, isDisabled) {
		...
		const androidStyles = {
			...
			lineHeight: CommonStyle.fontSizeXS + 8
		};
		...
		return (
			...
			>
				...
				<TextLoad
					...
					style={
						Platform.OS === 'android' ? androidStyles : iosStyles
					}
				>
					{titleDisplay}
				</TextLoad>
	...
	handleShowNewDetail(dataNews) {
		const { isDisableShowNewDetail, changeAllowUnmount, reset } = this.props;
		if (isDisableShowNewDetail) return;
		reset && reset()
	...
	renderFooter() {
		const { isMoreNews, isLoading } = this.props;
		if (isLoading) {
			return (
				<View
					style={{
	...
	render() {
		const { listNewsData, style, status } = this.props;
		...
		if (status) {
			return (
				...
				>
					<ErrorStatus status={status} title={I18n.t('News')} />
				</View>
			);
		}

		if (_.isEmpty(listNewsData) && !this.props.isLoading) return this.renderEmpty();
