...
import { changeDurationSL, changeDatePeriodSL } from '~/screens/new_order/Redux/actions.js'
...
const StopLossInputDate = ({
    ...
    const date = useSelector(state => state.newOrder.stopPrice.expiryTime, shallowEqual)
    const dispatch = useDispatch()
    const changeDate = useCallback((date) => {
        dispatch(changeDatePeriodSL(date))
    }, [])
    ...
    const setValue = useCallback((value) => {
        dispatch(changeDurationSL(value))
    }, [])
...
StopLossInputDate.defaultProps = {
    title: 'Default Label',
...
export default StopLossInputDate;
