...
import { changeDurationTP, changeDatePeriodTP } from '~/screens/new_order/Redux/actions.js'
...
const TakeProfitInputDate = ({
    ...
    const date = useSelector(state => state.newOrder.takeProfitLoss.expiryTime, shallowEqual)
    const dispatch = useDispatch()
    const changeDate = useCallback((date) => {
        dispatch(changeDatePeriodTP(date))
    }, [])
    ...
    const setValue = useCallback((value) => {
        dispatch(changeDurationTP(value))
    }, [])
...
TakeProfitInputDate.defaultProps = {
    title: 'Default Label',
...
export default TakeProfitInputDate;
