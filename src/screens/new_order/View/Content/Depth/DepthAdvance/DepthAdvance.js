	...
	const translateContent = useCallback(() => {
		setTimeout(() => {
			if (TabModel.model.depth === 'BID') {
				return refScrollView.current && refScrollView.current.scrollToEnd?.({ animated: true })
			}
			if (TabModel.model.depth === 'ASK') {
				return refScrollView.current && refScrollView.current.scrollTo?.({ x: 0, y: 0, animated: true })
			}
			console.info('translateContent', TabModel.model.rowDepthHeight)
			return refScrollView.current && refScrollView.current.scrollTo?.({ x: 0, y: TabModel.model.rowDepthHeight * 5, animated: true })
		}, 100);
	}, [])
	useEffect(() => {
	...
	const onChangeTab = useCallback((tab) => {
		...
		switch (tab.key) {
			...
			case 'ALL':
				TabModel.model.changeMaxHeight({ isShowAll: true })
				dic.current.timeoutScroll = setTimeout(() => {
					refScrollView.current.scrollTo?.({ x: 0, y: TabModel.model.rowDepthHeight * 5, animated: true })
				}, 100); break;
			case 'ASK':
				TabModel.model.changeMaxHeight({ isShowAll: false })
				dic.current.timeoutScroll = setTimeout(() => {
