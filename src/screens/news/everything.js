...
import { logAndReport, setStyleNavigation, switchForm, logDevice, showNewsDetail, openSignIn } from '../../lib/base/functionUtil';
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }} testID='newsScreen'>
                {
                    this.props.isConnected ? null : (Controller.getLoginStatus() ? null : <NetworkWarning />)
                }
                {
                ...
                }
                {
                    !Controller.getLoginStatus()
                        ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', backgroundColor: CommonStyle.backgroundColor }} >
                            <Text style={{ opacity: 0.87, color: CommonStyle.fontColor }}>{I18n.t('newsPart1')} </Text>
                            <Text testID={`signin`} style={{ color: '#007aff' }} onPress={() => openSignIn()}>{I18n.t('newsPart2')} </Text>
                            <Text style={{ opacity: 0.87, color: CommonStyle.fontColor }}>{I18n.t('newsPart3')}</Text>
                        </View>
                        : !this.dic.dataSource
                            ? <View />
                            : this.dic.dataSource.length
                                ? <ListView
                                    ...
                                    style={{ height: height, backgroundColor: CommonStyle.backgroundColor }}
                                />
