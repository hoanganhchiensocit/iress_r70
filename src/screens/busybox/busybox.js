...
export class BusyBox extends Component {
	...
	componentDidMount() {
		this.signInSuccess = EventEmitter.addListener(
			'signInSuccess',
			handleSignInOkta
		);
		this.signOutSuccess = EventEmitter.addListener(
			'signOutSuccess',
			handleSignOutOkta
		);
		this.onError = EventEmitter.addListener('onError', this.handleCancelOkta);
		this.onCancelled = EventEmitter.addListener(
			'onCancelled',
			this.handleCancelOkta
		);
		// Animation
		declareSequenceAnimation([
								...
								>
									<Text
										style={[
											...
											{ color: '#FFF', marginBottom: 4, textAlign: 'left' }
										]}
									>
