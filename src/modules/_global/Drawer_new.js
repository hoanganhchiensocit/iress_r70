...
import { useShadow as useShadowCustom } from '~/component/shadow/SvgShadowCustom';
...
import Icon2 from '~/screens/watchlist/Component/Icon2';
import { EventEmitter } from '@okta/okta-react-native';
import { getUnSubTopic } from '~/screens/alertLog/Model/AlertLogModel';
...
const { ROLE_USER, THEME, WIDTH_DRAWER } = ENUM;
			...
			<View
				...
				style={[
						...
						: CommonStyle.headerDrawerAndroid,
					{
						...
						backgroundColor: CommonStyle.color.bgAccount
					}
				]}
			>
				<Image
					style={{
						height: 122,
						width: 156,
						marginRight: (WIDTH_DRAWER - 156) / 2
					}}
...
const DataDisclaimer = ({ navigator }) => {
	const [Shadow, onLayout] = useShadowCustom();
	showDataDisclaimer = () => {
		navigator &&
				...
				>
					...
					>
						...
						<Ionicons
							...
							size={10}
							color={CommonStyle.color.modify}
							style={{ top: 1, marginLeft: 2, marginRight: 4 }}
						/>
					</TouchableOpacityOpt>
				</View>
				<View style={{ height: '100%', marginLeft: 4, paddingTop: 4 }}>
					<Image
						style={{ height: 31, width: 41 }}
						source={logo}
						resizeMode={'contain'}
					/>
				</View>
...
class Drawer extends XComponent {
	...
	switchTab(menuSelected, isBypass, initialPage = 0, isClose) {
		const index = [
			...
			ENUM.MENU_SELECTED.orders
		].indexOf(menuSelected);
		if (index > -1) {
			func.setTabActive(index);
		} else {
			func.setTabActive('');
		}
		...
		switch (menuSelected) {
			...
			case ENUM.MENU_SELECTED.activities:
				...
				break;
			case ENUM.MENU_SELECTED.alertLog:
				dataStorage.menuSelected = ENUM.MENU_SELECTED.alertLog;
				this.switchForm(menuSelected, 'equix.AlertLog', {});
				break;
		}
	}
	...
	renderAlertsLog = () => {
		if (!checkRoleByKey(ROLE_USER.ROLE_ALERT_LOG)) return null;
		const { afterLogin, menuSelected } = this.state;
		const selected = menuSelected === ENUM.MENU_SELECTED.alertLog;
		...
		return (
			<TouchableOpacity
				style={styles.midRowContainer}
				onPress={() => {
					this.switchTab(ENUM.MENU_SELECTED.alertLog);
				}}
			>
				<CommonStyle.icons.bell
					...
					color={
						selected
							? CommonStyle.btnOrderPositionBgColor
	...
	render() {
		...
		return (
			<View
				style={{
					flex: 1,
					backgroundColor: CommonStyle.color.bgDrawer,
					width: WIDTH_DRAWER
				}}
			>
				<View style={{ flex: 1 }}>
				...
				<View>
					<DataDisclaimer navigator={this.props.navigator} />
					<SignOutButton
						signOutFn={this.openModal}
						navigator={this.props.navigator}
					/>
				</View>
			</View>
